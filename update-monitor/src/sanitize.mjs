...
export function sanitizeValue(value, depth = 0) {
  ...
  return Object.fromEntries(Object.entries(value).slice(0, 100).map(([key, item]) => [key, SECRET_KEY.test(key) ? '[redacted]' : sanitizeValue(item, depth + 1)]))
}

export function safeError(error) {
  return { name: sanitizeText(error?.name || 'Error', 100), message: sanitizeText(error?.message || 'Unexpected error', 1_000), code: sanitizeText(error?.code || '', 100) }
}

export function log(event) {
  process.stdout.write(`${JSON.stringify(sanitizeValue({ at: new Date().toISOString(), ...event }))}\n`)
}
