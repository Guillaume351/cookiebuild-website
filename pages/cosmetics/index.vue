<template>
  <div class="space-y-16 pb-12">
    ...
    <p v-if="!purchaseEnabled" role="status" class="rounded-xl border border-orange-400/30 bg-orange-950/20 p-4 text-orange-100">Les achats sont actuellement indisponibles. Tu peux découvrir les effets et <NuxtLink to="/cosmetics/history" class="font-bold underline">consulter tes accès existants</NuxtLink> en liant ton joueur.</p>
    ...
    <section class="relative overflow-hidden rounded-3xl border border-orange-500/20 bg-gradient-to-br from-zinc-950 via-zinc-900 to-orange-950/40 px-6 py-14 md:px-12 md:py-20">
      <div class="relative z-10 max-w-3xl">
        <span class="inline-flex rounded-full border border-orange-400/40 bg-orange-500/10 px-3 py-1 text-sm font-black uppercase tracking-widest text-orange-200">{{ purchaseEnabled ? "Disponible sur le web" : "Prochainement" }}</span>
        <h1 class="mt-6 text-4xl font-black tracking-tight text-white md:text-6xl">Collection Supporter &amp; cosmétiques</h1>
        ...
        <div class="mt-7 flex flex-wrap gap-3">
          <NuxtLink to="/cosmetics/connect" class="rounded-xl bg-orange-500 px-5 py-3 font-black text-zinc-950 hover:bg-orange-400">Lier mon joueur</NuxtLink>
          <NuxtLink to="/cosmetics/history" class="rounded-xl border border-white/15 px-5 py-3 font-black text-white hover:border-orange-400">Inventaire et historique</NuxtLink>
        </div>
      </div>
    ...
    <section aria-labelledby="subscription-title" class="overflow-hidden rounded-3xl border-2 border-amber-300/50 bg-gradient-to-br from-amber-500/15 via-zinc-900 to-orange-950/50 p-6 shadow-[0_0_50px_rgba(245,158,11,.12)] md:p-9">
      <div class="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
        ...
        <div class="min-w-56 rounded-2xl border border-white/10 bg-black/30 p-6 text-center">
          ...
          <NuxtLink v-if="purchaseEnabled" :to="`/cosmetics/checkout?product=${subscriptionProduct.id}`" class="mt-5 block min-h-11 w-full rounded-xl bg-amber-300 px-4 py-3 font-black text-zinc-950 hover:bg-amber-200">S’abonner et payer {{ euros(subscriptionProduct.priceTtcCents) }} / mois</NuxtLink>
    ...
    <section aria-labelledby="collection-title">
      <div class="max-w-3xl">
        ...
        <h2 id="collection-title" class="mt-2 text-3xl font-black text-white">Les sept éléments</h2>
    ...
    <section aria-labelledby="offers-title">
      ...
      <div class="mt-8 grid gap-5 lg:grid-cols-3">
        <article v-for="product in oneTimeProducts" :key="product.id" class="flex flex-col rounded-3xl border border-zinc-800 bg-zinc-900 p-6" :class="product.kind === 'collection_pack' ? 'ring-2 ring-orange-500/50' : ''">
          ...
          <NuxtLink v-if="purchaseEnabled" :to="`/cosmetics/checkout?product=${product.id}`" class="mt-5 min-h-11 rounded-xl bg-orange-500 px-4 py-3 text-center font-black text-zinc-950 hover:bg-orange-400">Acheter et payer {{ euros(product.priceTtcCents) }}</NuxtLink>
        ...
        <div class="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
          ...
          <div class="flex flex-wrap gap-3">
            <template v-for="support in supportProducts" :key="support.id">
              <NuxtLink v-if="purchaseEnabled" :to="`/cosmetics/checkout?product=${support.id}`" class="min-h-11 rounded-xl border border-sky-500/20 bg-sky-950/40 px-5 py-3 font-black text-sky-100 hover:border-sky-300">Soutenir à {{ euros(support.priceTtcCents) }}</NuxtLink>
...
useSeoMeta({
  title: () => purchaseEnabled.value ? "Cosmétiques et Supporter | Cookie Build" : "Cosmétiques — bientôt | Cookie Build",
  description: () => purchaseEnabled.value ? "Découvre les avantages sociaux et de lobby Cookie Build, sans avantage compétitif." : "Prévisualise la première collection Cookie Build, sans avantage compétitif ni paiement actif.",
  robots: "index, follow",
});
</script>
