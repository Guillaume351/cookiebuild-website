...
const liveSnapshots = computed(() => snapshots.value.filter((snapshot) => observedNow.value - new Date(snapshot.observedAt).getTime() < 20_000));
...
async function sendPlayerCommand() { const form = playerForm.value; if (!form) return; const payload = form.type === "message_player" ? { message: form.text } : { reason: form.text, player_name: form.player.name, ...(["ban_temp", "mute"].includes(form.type) ? { duration_seconds: form.duration } : {}) }; if (await command({ type: form.type, serverId: form.player.serverId, targetId: form.player.id, payload })) playerForm.value = null; }
...
onMounted(() => { timer = setInterval(() => { observedNow.value = Date.now(); void load(true); }, 3_000); }); onUnmounted(() => { if (timer) clearInterval(timer); });
await load();
</script>
