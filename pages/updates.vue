<template>
  <div class="mx-auto max-w-5xl space-y-10">
    ...
    <NuxtLink :to="locale.code === 'fr' ? '/fr/updates/nomad-wars-preview' : '/updates/nomad-wars-preview'" class="block rounded-3xl border border-orange-500/30 bg-zinc-900 p-7 transition hover:border-orange-400">
      <p class="text-sm font-bold uppercase tracking-wider text-orange-400">{{ locale.code === 'fr' ? 'Bêta publique · Jouable maintenant' : 'Public beta · Play now' }}</p>
      <h2 class="mt-3 text-3xl font-black text-white">{{ locale.code === 'fr' ? 'Nomad Wars : la bêta est ouverte' : 'Nomad Wars: the beta is open' }}</h2>
      <p class="mt-4 text-zinc-300">{{ locale.code === 'fr' ? 'Des zones sûres mobiles, un combat final et trois mondes à explorer en 3D. Lis les règles et rejoins la bêta.' : 'Moving safe zones, a final showdown and three worlds to explore in 3D. Read the rules and join the beta.' }}</p>
      <span class="mt-4 inline-flex min-h-11 items-center font-bold text-orange-300">{{ locale.code === 'fr' ? 'Lire les règles de la bêta' : 'Read the beta rules' }} →</span>
    </NuxtLink>
    <section
      v-if="nextEvent"
