<template>
  <div class="space-y-8">
    ...
    <section v-if="map.districts.length" aria-labelledby="districts-title"><h2 id="districts-title" class="mb-5 text-2xl font-black text-white">Four districts. Different opportunities.</h2><div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div v-for="district in map.districts" :key="district.name" class="rounded-2xl border border-zinc-800 bg-zinc-900 p-5"><p class="text-sm font-bold uppercase tracking-wider text-orange-400">{{ district.role }}</p><h3 class="mt-2 text-xl font-bold text-white">{{ district.name }}</h3></div></div><p class="mt-5 text-zinc-400">Districts have different specialties. Everyone starts with basic supplies, and the workshop is optional. Layouts and resource quantities are being tuned for the game.</p></section>
