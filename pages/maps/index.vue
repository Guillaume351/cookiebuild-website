<template>
  <div class="space-y-12">
    <header class="max-w-3xl py-8">
      ...
      <p class="mt-4 text-zinc-400">Browse the published map snapshots by game, including three upcoming Nomad Wars worlds. Availability follows each game’s current rotation.</p>
    </header>
    ...
    <section v-for="group in groups" :key="group.game" :aria-label="`${group.game} maps`">
      <div class="mb-6 flex flex-wrap items-end justify-between gap-4"><h2 class="text-3xl font-black text-white">{{ group.game }}{{ group.game === 'Nomad Wars' ? ' · Coming soon' : '' }}</h2><NuxtLink v-if="group.game === 'Nomad Wars'" to="/nomad-wars" class="inline-flex min-h-11 items-center font-bold text-orange-300">Discover the game →</NuxtLink></div>
...
useSeoMeta({ title: "Explore our Minecraft maps | Cookie Build", description: "Explore real Cookie Build Minecraft maps in 3D. Discover three upcoming Nomad Wars worlds, their districts and central arenas.", ogImage: "https://www.cookie-build.com/maps/nomad-oasis.webp" });
