...
function boundedText(
  ...
  if (typeof value !== "string")
    throw new PermanentOutboxError(`${field} must be a string`);
  const text = value.trim();
  if (!text && required) throw new PermanentOutboxError(`${field} is required`);
  if (text.length > maximum)
    throw new PermanentOutboxError(`${field} is too long`);
  return text || undefined;
}

export function preferenceKind(
  kind: string,
): NotificationPreferenceKind | undefined {
  if (
    ["announcement", "announcements", "news", "news_published"].includes(kind)
  ) {
    return "announcement";
  }
  ...
  if (["social", "friend_request", "party_invite"].includes(kind))
    return "social";
  ...
  if (kind === "skyblock_market_sold") return "skyblock_market_sold";
  if (kind === "skyblock_worker_full") return "skyblock_worker_full";
  if (kind === "skyblock_objective_ready") return "skyblock_objective_ready";
  return undefined;
}
...
export function parseNotificationAudience(
  ...
  for (const key of ["mobileUserIds", "deviceIds"] as const) {
    ...
    if (
      ...
      candidate.length === 0 ||
      candidate.length > 5_000
    ) {
      throw new PermanentOutboxError(
        `audience.${key} must contain 1 to 5000 UUIDs`,
      );
    }
    ...
    if (
      ...
      throw new PermanentOutboxError(
        `audience.${key} contains an invalid UUID`,
      );
    }
    audience[key] = ids;
    selectors += 1;
  ...
  if (selectors !== 1) {
    throw new PermanentOutboxError(
      "audience must contain exactly one selector",
    );
  }
  const internalRallyRetry =
    allowInternalDeviceRetry && Boolean(audience.deviceIds);
...
function safeDeepLink(value: unknown) {
  ...
  const permittedWebHost =
    url.protocol === "https:" &&
    (url.hostname === "cookie-build.com" ||
      url.hostname === "www.cookie-build.com");
  if (url.protocol !== "cookiebuild:" && !permittedWebHost) {
    throw new PermanentOutboxError(
      "payload.deepLink uses an unsupported origin",
    );
  }
  return text;
}
...
function rallyInteger(value: unknown, field: string, minimum: number) {
  if (
    ...
    throw new PermanentOutboxError(
      `payload.${field} must be an integer from ${minimum} to 1000`,
    );
  }
  return value;
}
...
export function parsePlayerRallyPayload(value: unknown): NotificationPayload {
  ...
  if (
    ...
    throw new PermanentOutboxError(
      "payload.source must be login, player, or automatic",
    );
  }
  if (
  ...
  if (input.source === "login") {
    if (input.gamemode !== "network") {
      throw new PermanentOutboxError(
        "payload.gamemode must be network for login rallies",
      );
    }
    ...
    if (typeof input.actorDisplayName !== "string") {
      throw new PermanentOutboxError(
        "payload.actorDisplayName is required for login rallies",
      );
    }
    actorDisplayName = input.actorDisplayName.trim().normalize("NFKC");
    if (!PLAYER_RALLY_ACTOR_PATTERN.test(actorDisplayName)) {
      throw new PermanentOutboxError(
        "payload.actorDisplayName is not a public player name",
      );
    }
  } else if (input.gamemode === "network") {
    throw new PermanentOutboxError(
      "payload.gamemode network is reserved for login rallies",
    );
  } else if (input.source === "player") {
    if (neededCount < 1) {
      throw new PermanentOutboxError(
        "payload.neededCount must be positive for player rallies",
      );
    }
    if (typeof input.actorDisplayName !== "string") {
      throw new PermanentOutboxError(
        "payload.actorDisplayName is required for player rallies",
      );
    }
    actorDisplayName = input.actorDisplayName.trim().normalize("NFKC");
    if (!PLAYER_RALLY_ACTOR_PATTERN.test(actorDisplayName)) {
    ...
    throw new PermanentOutboxError(
      "payload.actorDisplayName must be null for automatic rallies",
    );
  }
  if (input.source !== "login" && neededCount === 0 && queuedCount === 0) {
    throw new PermanentOutboxError(
      "payload.queuedCount must be positive for a starting game",
    );
  }
...
export function parseNotificationPayloadForKind(
  ...
  if (!sourceData)
    throw new PermanentOutboxError("payload.data must be an object");
...
function offsetMinutes(date: Date, timezoneOffsetMinutes: number | null) {
  ...
  return (
    (((utcMinutes + timezoneOffsetMinutes) % (24 * 60)) + 24 * 60) % (24 * 60)
  );
}
...
export function isInQuietHours(
  ...
  const now =
    (timezone ? localMinutes(date, timezone) : undefined) ??
