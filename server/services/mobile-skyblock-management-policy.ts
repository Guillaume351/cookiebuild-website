...
export function skyblockBuildRadiusForTier(tier: number) {
  if (!Number.isSafeInteger(tier) || tier < 1 || tier > SKYBLOCK_MAX_GENERATOR_TIER) {
    throw new Error("Invalid Skyblock generator tier");
  }
  const configured = policy.generator.tiers.find((candidate) => candidate.tier === tier);
...
export function skyblockGeneratorUpgradeCost(nextTier: number) {
  const cost = policy.generator.tiers.find((candidate) => candidate.tier === nextTier)?.upgradeCostCoins;
  if (cost === undefined || cost === null) throw new Error("Invalid Skyblock generator upgrade tier");
  return cost;
}
...
export const SKYBLOCK_QUESTS: readonly SkyblockQuestDefinition[] = policy.quests;
