...
export const SKYBLOCK_CATEGORIES = ["mining", "farming", "foraging", "combat"] as const;
export const SKYBLOCK_RARITIES = ["common", "uncommon", "rare", "epic"] as const;
...
const ITEMS_BY_ID = new Map(SKYBLOCK_ITEM_CATALOG.map((item) => [item.id, item]));

export function skyblockCatalogItem(itemId: string) {
  return ITEMS_BY_ID.get(itemId);
}

export function skyblockCatalogItemsForCategory(category: SkyblockCategory | null) {
  return category
