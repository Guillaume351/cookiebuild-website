<template>
  <article :lang="language" class="space-y-12 sm:space-y-16">
    <div class="flex flex-wrap items-center justify-between gap-4">
      <NuxtLink v-if="article" :to="localizePath('/updates')" class="inline-flex min-h-11 items-center font-bold text-amber-300">← {{ copy.back }}</NuxtLink>
    ...
    <BetaPlayInstructions command="/fatking play" />
    ...
    <section class="grid gap-8 lg:grid-cols-2">
      <div class="rounded-3xl border border-amber-500/25 bg-gradient-to-br from-zinc-900 to-amber-950/30 p-7 sm:p-9"><h2 class="text-2xl font-black text-white">{{ copy.riskTitle }}</h2><p class="mt-5 leading-relaxed text-zinc-300">{{ copy.risk }}</p><div class="mt-7 grid grid-cols-3 gap-3" :aria-label="ui.gold"><div v-for="sample in [{gold:1,points:1},{gold:16,points:4},{gold:64,points:8}]" :key="sample.gold" class="rounded-xl border border-amber-500/20 bg-black/25 p-4 text-center"><p class="text-3xl font-black text-amber-300">{{ sample.gold }}</p><p class="text-xs text-zinc-300">{{ ui.ingots }}</p><p class="mt-3 font-bold text-white">{{ sample.points }} pt/s</p></div></div></div>
    ...
    <section aria-labelledby="fat-objectives"><h2 id="fat-objectives" class="mb-7 text-3xl font-black text-white sm:text-4xl">{{ copy.objectivesTitle }}</h2><div class="grid gap-5 lg:grid-cols-3"><div v-for="objective in copy.objectives" :key="objective.title" class="rounded-3xl border border-amber-500/20 bg-zinc-900 p-7"><h3 class="text-xl font-bold text-amber-300">{{ objective.title }}</h3><p class="mt-4 leading-relaxed text-zinc-300">{{ objective.text }}</p></div></div></section>
    <section id="fat-king-map" class="scroll-mt-24"><h2 class="text-3xl font-black text-white sm:text-4xl">{{ copy.mapsTitle }}</h2><p class="mb-7 mt-4 max-w-3xl text-lg text-zinc-300">{{ copy.mapsIntro }}</p><MapWorldViewer :map="map" /><div class="mt-5 flex flex-wrap gap-6"><NuxtLink :to="localizePath(`/maps/${map.slug}`)" class="inline-flex min-h-11 items-center font-bold text-amber-300">{{ copy.mapLink }} →</NuxtLink><NuxtLink :to="localizePath('/maps')" class="inline-flex min-h-11 items-center font-bold text-zinc-300">{{ copy.atlas }} →</NuxtLink></div><details class="mt-5 rounded-2xl border border-zinc-800 bg-zinc-900 p-4"><summary class="cursor-pointer py-2 font-bold text-zinc-200">{{ copy.tiny }}</summary><img src="/maps/fat-king-crown-top.webp" :alt="copy.tiny" width="1200" height="900" loading="lazy" class="mt-4 w-full rounded-xl" /></details></section>
...
const language = computed(() => locale.value.code);
...
useLocalizedSeo(stripSiteLocale(route.path), () => `${props.article ? copy.value.articleTitle : 'Fat King'} — ${ui.value.beta} | Cookie Build`, () => copy.value.intro);
useSeoMeta({ ogImage: 'https://www.cookie-build.com/maps/fat-king-crown.webp', twitterImage: 'https://www.cookie-build.com/maps/fat-king-crown.webp', ogImageAlt: () => copy.value.mapsTitle, twitterImageAlt: () => copy.value.mapsTitle });
</script>
