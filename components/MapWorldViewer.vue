<template>
  <section class="overflow-hidden rounded-3xl border border-zinc-700 bg-zinc-950" :aria-label="`${map.name} interactive map`">
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 p-4">
      ...
      <button v-if="active" type="button" class="inline-flex min-h-11 items-center rounded-xl border border-zinc-600 px-3 text-sm font-bold text-zinc-200 hover:border-orange-400" @click="active = false">{{ french ? 'Revenir à l’image fixe' : 'Show still image' }}</button>
      <a :href="viewerUrl" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-11 items-center text-sm font-bold text-orange-300">{{ french ? 'Plein écran' : 'Open full screen' }} ↗</a>
    </div>
    ...
    <div class="relative h-[min(70vh,700px)] min-h-[360px]">
      ...
      <template v-else>
        ...
        <div class="absolute inset-0 flex items-center justify-center bg-black/20 p-6">
          <button type="button" class="min-h-14 rounded-2xl bg-orange-600 px-7 py-4 font-black text-white shadow-xl hover:bg-orange-500 focus-visible:outline focus-visible:outline-2 focus-visible:outline-white" @click="active = true">{{ french ? (topDown ? 'Explorer la vue du dessus' : 'Explorer en 3D') : (topDown ? 'Explore in top-down view' : 'Explore in 3D') }}</button>
        </div>
      </template>
    </div>
    <p class="border-t border-zinc-800 p-4 text-sm leading-relaxed text-zinc-400">{{ french ? 'Rendu du vrai monde Minecraft. Déplace la vue et zoome pour explorer les bâtiments. La vue interactive se charge à la demande et nécessite WebGL. Si elle ne se charge pas, ' : 'Real Minecraft world snapshot. Drag to explore and zoom to inspect the buildings. The interactive view loads only when you open it; it needs WebGL. If it does not load, ' }}<a :href="map.image" class="text-orange-300 underline">{{ french ? 'ouvre l’image fixe' : 'open the still image' }}</a>.<span v-if="map.game === 'Nomad Wars'"> {{ french ? 'Les zones mobiles du jeu ne sont pas affichées dans cette préversion.' : 'Moving gameplay zones are not shown in this map preview.' }}</span></p>
  </section>
</template>
...
const views = computed(() => [{ label: french.value ? "Monde 3D" : "3D world", topDown: false }, { label: "Tiny map", topDown: true }]);
