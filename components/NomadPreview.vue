<template>
  <article :lang="language" class="space-y-14">
    <div class="flex flex-wrap items-center justify-between gap-4"><NuxtLink v-if="article" :to="localizePath('/updates')" class="inline-flex min-h-11 items-center font-bold text-orange-300">← {{ ui.back }}</NuxtLink><span v-else class="text-sm font-bold uppercase tracking-[0.2em] text-zinc-400">Cookie Build / Nomad Wars</span></div>
    <header class="relative isolate overflow-hidden rounded-[2rem] border border-orange-500/20 bg-zinc-950">
      <img src="/maps/nomad-oasis.webp" :alt="`Nomad Wars — ${ui.image}: Sunken Oasis`" fetchpriority="high" width="1200" height="900" class="absolute inset-0 -z-20 h-full w-full object-cover opacity-60" />
    ...
    <BetaPlayInstructions command="/nomadwars play" />
    ...
    <section class="rounded-3xl border border-orange-500/25 bg-gradient-to-br from-zinc-900 to-orange-950/30 p-7 sm:p-10"><h2 class="text-3xl font-black text-white">{{ copy.feedbackTitle }}</h2><p class="mt-5 max-w-3xl text-lg leading-relaxed text-zinc-300">{{ copy.feedback }}</p><div class="mt-6 flex flex-wrap gap-5"><a href="https://discord.gg/ajmPnwh9g8" target="_blank" rel="noopener noreferrer" class="inline-flex min-h-11 items-center font-bold text-orange-300">{{ copy.discord }} ↗</a><NuxtLink v-if="!article" :to="localizePath('/updates/nomad-wars-preview')" class="inline-flex min-h-11 items-center font-bold text-white">{{ copy.article }} →</NuxtLink></div></section>
  </article>
</template>
...
useLocalizedSeo(stripSiteLocale(route.path), () => `Nomad Wars — ${ui.value.beta} | Cookie Build`, () => copy.value.intro);
